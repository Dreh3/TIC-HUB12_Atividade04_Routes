...
<template>
  ...
  <section class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-2 bg-gray-100 p-4">
      <div v-for="product in products" :key="product.id">
        <div >
          ...
          <Button @click="goToDetail(product)">Detalhes</Button>
          <!-- usa o alerta click -->
        </div>
      </div>
  </section>
  ...
  <div v-if="cart.listProducts.length >0">
    <DataView :value="cart.listProducts">
      <template #list="slotProps">
        <section class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-2 bg-gray-100 p-4">
          <div v-for="(item,index) in slotProps.items" :key="index" >
            <!-- se passar cart e index e faz uma função para obter a quantidade -->
            <CartItemCard :item="item" @deletar="delItem" @add-item="addItem" @dec-item="decItem"/> 
